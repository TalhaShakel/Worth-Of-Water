...
class HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    ...
    AppGlobal.eventBus.on<RefreshUserDetails>().listen((event) async {
      ...
      setState(() {
        goalValue = StringHelper.getData(
            AppGlobal.DAILY_WATER_VALUE.toInt().toString() +
                " " +
                AppGlobal.WATER_UNIT_VALUE);
      });
      ...
      String path =
          await SharedPref.readPreferenceValue(USER_PHOTO, PrefEnum.STRING) ??
              "";
  ...
  loadSelectedContainer() async {
    String unit =
        await SharedPref.readPreferenceValue(WATER_UNIT, PrefEnum.STRING);
    //print(unit);
    //print(selected_pos);
    if (unit == "ml") {
      setState(() {
        selectedContainerLabel =
            lstContainer[selected_pos].containerValue.toString() + " " + unit;
        selectedContainerImage =
            getImage(int.parse(lstContainer[selected_pos].containerValue));
      });
    } else {
      setState(() {
        //print("-====");
        ...
        selectedContainerLabel =
            lstContainer[selected_pos].containerValueOZ.toString() + " " + unit;
        selectedContainerImage =
            getImageOz(int.parse(lstContainer[selected_pos].containerValue));
      });
    }
  }
          ...
          children: [
            InkWell(
              onTap: () {
                ...
                setState(() {
                  if (intl.DateFormat("yyyy-MM-dd").format(selectedDate) ==
                      intl.DateFormat("yyyy-MM-dd").format(yesterdayDate)) {
            ...
            InkWell(
              onTap: () {
                ...
                setState(() {
                  if (intl.DateFormat("yyyy-MM-dd").format(selectedDate) ==
                      intl.DateFormat("yyyy-MM-dd").format(todayDate)) {
                    lblDate = buildTranslate(context, "today");
                  } else if (intl.DateFormat("yyyy-MM-dd")
                          .format(selectedDate) ==
                      intl.DateFormat("yyyy-MM-dd").format(yesterdayDate)) {
                    lblDate = buildTranslate(context, "yesterday");
                  } else {
                      ...
                      Stack(
                        children: [
                          RotationTransition(
                            ...
                            child: Image(
                              image:
                                  AssetsHelper.getAssetImage("menu_header.png"),
                              height: 150,
                              fit: BoxFit.fill,
                            ),
                          ),
        ...
        children: [
          Expanded(
            ...
            child: ButtonView(
              ...
              height: 50,
              onPressed: () {
                launchWhatsApp2(phone: 65656565, message: "message");
  ...
  void launchWhatsApp2({
    @required int phone,
    @required String message,
  }) async {
    String url() {
      if (Platform.isAndroid) {
        return "https://wellwisher.world/";
      } else {
  ...
  void launchWhatsApp({
    ...
    String url() {
      if (Platform.isAndroid) {
        return "https://api.whatsapp.com/send?phone=919909159999";
      } else {
        return "whatsapp://send?   phone=$phone&text=${Uri.parse(message)}";
      }
    }
  ...
  void getMenuList() async {
    ...
    lstMenuList.clear();
    lstMenuList.add(new Menu(
        menuName: buildTranslate(context, "home"), isSelected: true, index: 0));
    lstMenuList.add(new Menu(
        menuName: buildTranslate(context, "drink_history"),
        isSelected: false,
        index: 1));
    lstMenuList.add(new Menu(
        menuName: buildTranslate(context, "drink_report"),
        isSelected: false,
        index: 2));
    lstMenuList.add(new Menu(
        menuName: buildTranslate(context, "settings"),
        isSelected: false,
        index: 4));
    lstMenuList.add(new Menu(
        menuName: buildTranslate(context, "faqs"),
        isSelected: false,
        index: 5));
    lstMenuList.add(new Menu(
        menuName: buildTranslate(context, "privacy_policy"),
        isSelected: false,
        index: 6));
    lstMenuList.add(new Menu(
        menuName: buildTranslate(context, "tell_a_friend"),
        isSelected: false,
        index: 7));
  ...
  void openScreen(int index) {
    switch (index) {
      ...
      case 6:
        ...
        break;
      case 7:
        String str = buildTranslate(context, "app_share_txt")
            .replaceAll("#1", AppGlobal.APP_SHARE_URL);
        Share.share(str);
        break;
    }
  }
          ...
          Align(
            ...
            child: Column(
              children: [
                InkWell(
                  onTap: () {
                    NavigatorHelper.openDialog(ContainerListScreen(
                      lstContainer: lstContainer,
                      callback: (ContainerModel container, int index) {
                        setState(() {
                          selected_pos = index;
                          SharedPref.savePreferenceValue(SELECTED_CONTAINER,
                              int.parse(container.containerId));
                  ...
                  child: Container(
                    ...
                    child: Padding(
                      ...
                      child: Image(
                        image:
                            AssetsHelper.getAssetIcon(selectedContainerImage),
                      ),
                    ),
                  ),
                ),
          ...
          Align(
            ...
            child: Column(
              children: [
                ...
                Text(
                  buildTranslate(context, "history"),
                  style: Fonts.homeBlueLightSmallTextStyle,
                ),
              ],
            ),
                  ...
                  onEnd: () {
                    ...
                    if (oldDrinkWater < AppGlobal.DAILY_WATER_VALUE) {
                      if (drinkWater >= AppGlobal.DAILY_WATER_VALUE) {
                        NavigatorHelper.openDialog(DailyGoalReachedDialog(
                          drinkWater: drinkWater.toInt(),
                        ));
                      }
                    }
  ...
  countDrink(bool fromInit) async {
    ...
    final allRows = await AppGlobal.dbHelper.queryWhere(
        DatabaseHelper.tableDrinkDetails,
        "DrinkDate ='" +
            intl.DateFormat(AppGlobal.DATE_FORMAT).format(selectedDate) +
            "'");
    ...
    setState(() {
      consumedValue = StringHelper.getData(
          drinkWater.toInt().toString() + " " + AppGlobal.WATER_UNIT_VALUE);
      goalValue = StringHelper.getData(
          AppGlobal.DAILY_WATER_VALUE.toInt().toString() +
              " " +
              AppGlobal.WATER_UNIT_VALUE);
  ...
  refresh_bottle(bool isFromCurrentProgress, bool fromInit) {
    setState(() {
      ...
      //print(_height);
      if (_height > _maxHeight - 10) _height = _maxHeight - 10;
  ...
  addWater() async {
    ...
    if (AppGlobal.WATER_UNIT_VALUE == "ml")
      count_drink_after_add_current_water +=
          double.parse("" + lstContainer[selected_pos].containerValue);
    else if (!(AppGlobal.WATER_UNIT_VALUE == "ml"))
      count_drink_after_add_current_water +=
          double.parse("" + lstContainer[selected_pos].containerValueOZ);

    if (AppGlobal.WATER_UNIT_VALUE == "ml" &&
        count_drink_after_add_current_water > 8000) {
      if (drinkWater >= 8000)
        NavigatorHelper.openDialog(DailyMaxReachedDialog());
      else if (AppGlobal.DAILY_WATER_VALUE <
          (8000 - double.parse("" + lstContainer[selected_pos].containerValue)))
        NavigatorHelper.openDialog(DailyMaxReachedDialog());
    } else if ((!(AppGlobal.WATER_UNIT_VALUE == "ml")) &&
    ...
    if (!(await (SharedPref.readPreferenceValue(
        DISABLE_SOUND_WHEN_ADD_WATER, PrefEnum.BOOL)))) {
      player.stop();
      player.play();
    }
    ...
    Map<String, dynamic> params = {
      ...
      'ContainerValueOZ':
          lstContainer[selected_pos].containerValueOZ.toString(),
      'DrinkDate': intl.DateFormat(AppGlobal.DATE_FORMAT).format(selectedDate),
      'DrinkTime': intl.DateFormat("HH:mm").format(selectedDate),
      'DrinkDateTime':
          intl.DateFormat(AppGlobal.DATE_FORMAT).format(selectedDate) +
              " " +
              intl.DateFormat("HH:mm").format(selectedDate),
    };

    if (AppGlobal.WATER_UNIT_VALUE == "ml") {
      params["TodayGoal"] = AppGlobal.DAILY_WATER_VALUE.toString();
      params["TodayGoalOZ"] =
          HeightWeightHelper.mlToOzConverter(AppGlobal.DAILY_WATER_VALUE)
              .toString();
    } else {
      params["TodayGoal"] =
          HeightWeightHelper.ozToMlConverter(AppGlobal.DAILY_WATER_VALUE)
              .toString();
  ...
  getContainerList() async {
    ...
    int id =
        await SharedPref.readPreferenceValue(SELECTED_CONTAINER, PrefEnum.INT);
    ...
    final allRows = await AppGlobal.dbHelper
        .queryAllBySort(DatabaseHelper.tableContainer, "IsCustom DESC");
    //print('query all rows: ' + allRows.length.toString());
    ...
    for (int k = 0; k < allRows.length; k++) {
      ...
      setState(() {
        ContainerModel container = new ContainerModel(
          ...
          isSelected:
              row["ContainerID"].toString() == selectedContainerId.toString(),
          isCustom: row["IsCustom"].toString() == "1",
        );
        lstContainer.add(container);

        if (container.isSelected) selected_pos = k;
      });
    }
  ...
  getAllReminderData() async {
    ...
    bool isManual =
        await SharedPref.readPreferenceValue(IS_MANUAL_REMINDER, PrefEnum.BOOL);

    final allRows =
        await AppGlobal.dbHelper.queryAllRows(DatabaseHelper.tableAlarmDetails);
    ...
    for (int j = 0; j < allRows.length; j++) {
      ...
      if (row["AlarmType"] == "R") {
        if (!isManual) {
          final allInnerRows = await AppGlobal.dbHelper.queryWhere(
              DatabaseHelper.tableAlarmSubDetails,
              "SuperId='" + row["id"].toString() + "'");
          for (int k = 0; k < allInnerRows.length; k++) {
            Map<String, dynamic> innerRow = allInnerRows[k];
            ...
            DateTime dateTime =
                intl.DateFormat("hh:mm a").parse(innerRow["AlarmTime"]);
            ...
            lstReminder.add(NextReminderModel(
              time: innerRow["AlarmTime"],
              millisecond: dateTime2
                  .millisecondsSinceEpoch, //intl.DateFormat("hh:mm a").parse(innerRow["AlarmTime"]).millisecondsSinceEpoch
            ));
          }
        }
      } else {
        if (isManual) {
          ...
          if (row["IsOff"] == 0) {
            ...
            DateTime dt2 = DateTime.parse(
                intl.DateFormat("yyyy-MM-dd").format(DateTime.now()) +
                    " " +
                    intl.DateFormat("HH:mm:ss").format(dt));
            lstReminder.add(NextReminderModel(
                time: row["AlarmTime"],
                millisecond: dt2.millisecondsSinceEpoch));
          }
    ...
    if (lstReminder.length > 0) {
      nextReminderLabel = buildTranslate(context, "next_reminder")
          .replaceAll("\$1", lstReminder[tmp_pos].time);
    }
  }
