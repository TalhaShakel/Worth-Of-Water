...
class SplashScreenState extends State<SplashScreen> {
  ...
  void navigationPage() async {
    ...
    var _isLogin = await SharedPref.readPreferenceValue(
        HIDE_WELCOME_SCREEN, PrefEnum.BOOL);

    if (_isLogin) {
      NavigatorHelper.replace(HomeScreen());
    } else {
      NavigatorHelper.replace(MyHomePage());
    }
  }
  ...
  loadData() async {
    double tmpDailyWater =
        (await SharedPref.readPreferenceValue(DAILY_WATER, PrefEnum.DOUBLE)) ??
            0.0;
    ...
    await SharedPref.savePreferenceValue(
        WATER_UNIT, AppGlobal.WATER_UNIT_VALUE);
  }
                    ...
                    Expanded(
                      child: Center(
                        child: Column(
                          ...
                          children: [
                            Image(
                              image: AssetsHelper.getAssetLogo("wel.png"),
                              height: 220,
                              width: 220,
                            ),
                            SizedBox(
                            ...
                            Text(
                              buildTranslate(context, "appName"),
                              style: TextStyle(
